...
import { fetchAvaiableFleetData, fetchTraveledKmData } from '../api/dashboardService';
import { AvaiableFleetData, dappResponseData, TraveledKmData } from '../types/DashboardData';
...
const Dashboard: React.FC = () => {
  ...
  const TraveledKmChartData = {
    labels: traveledKm?.map((dado) => dado.line_id) || [],
    datasets: [
      {
        label: 'Distância Prevista',
        data: traveledKm?.map((dado) => dado.expected_traveled_distance_km) || [],
        backgroundColor: 'rgba(0, 0, 255, 0.2)',
        borderColor: 'rgba(0, 0, 255, 1)',
        borderWidth: 1,
      },
      {
        ...
        data: traveledKm?.map((dado) => dado.recorded_travel_distance_km) || [],
        backgroundColor: 'rgba(255, 0, 0, 0.2)',
        borderColor: 'rgba(255, 0, 0, 1)',
        borderWidth: 1,
      },
